\newacronym{ieee}{IEEE}{Institute of Electrical and Electronics Engineers}
\newacronym{tfg}{TFG}{Trabajo Final de Grado}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Añadidos %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newacronym{xdp}{XDP}{eXpress Data Path}
\newacronym{sbc}{SBC}{Single Board Computer}
\newacronym{ble}{BLE}{Bluetooth Low Energy}
\newacronym{uah}{UAH}{Universidad de Alcalá}
\newacronym{ebpf}{eBPF}{extended Berkeley Packet Filter}
\newacronym{bpf}{BPF}{Berkeley Packet Filter}
\newacronym{elf}{ELF}{Executable and Linkable Format}
\newacronym{bmv2}{BMv2}{Behavioral Model}
\newacronym{veth}{Veth}{Virtual Ethernet Device}
\newacronym{ap}{AP}{Access Point}
\newacronym{nic}{NIC}{Network Interface Card}
\newacronym{tc}{TC}{Traffic control}
\newacronym{gui}{GUI}{Graphical User Interface}
\newacronym{cli}{CLI}{Command Line Interface}
\newacronym{dos}{DoS}{Denial of Service}
\newacronym{onf}{ONF}{Open Networking Foundation}
\newacronym{wsn}{WSN}{Wireless Sensor Networks}
\newacronym{rssi}{RSSI}{Received Signal Strength Indicator}
\newacronym{lln}{LLN}{Low power and Lossy Networks}
\newacronym{ietf}{IETF}{Internet Engineering Task Force}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newacronym{tfm}{TFM}{Trabajo Fin de Máster}
\newacronym{6g}{6G}{Sexta Generación de redes móviles}
\newacronym{5g}{5G}{Quinta Generación de redes móviles}
\newacronym{ai}{AI}{\textit{Artificial Intelligence}}
\newacronym{sdn}{SDN}{Software-Defined Networking}
\newacronym{iot}{IoT}{Internet of Things}
\newacronym{m2m}{M2M}{Machine to Machine}
\newacronym{embb}{eMBB}{\textit{enhanced Mobile BroadBand}}
\newacronym{mmtc}{mMTC}{\textit{massive Machine-Type Communication}}
\newacronym{ml}{ML}{\textit{Machine Learning}}
\newacronym{urllc}{URLLC}{\textit{Ultra-Reliable and Low-Latency Communication}}
\newacronym{qos}{QoS}{Calidad de servicio}
\newacronym{mimo}{MIMO}{\textit{Multiple-Input and Multiple-Output}}
\newacronym{p4}{P4}{Programming Protocol-independent Packet Processors}
\newacronym{lldp}{LLDP}{Link Layer Discovery Protocol}
\newacronym{sbi}{SBI}{\textit{Southbound Interface}}
\newacronym{nbi}{NBI}{\textit{Northbound Interface}}
\newacronym{snmp}{SNMP}{Simple Network Management Protocol}
\newacronym{fifo}{FIFO}{\textit{First In, First Out}}
\newacronym{onos}{ONOS}{Open Network Operating System}
\newacronym{ovs}{OvS}{Open vSwitch}
\newacronym{bofus}{BOFUSS}{Basic OpenFlow User-space Software Switch}
\newacronym{itu}{ITU}{\textit{International Telecommunication Union}}
\newacronym{fl}{FL}{\textit{Federated Learning}}
\newacronym{rpl}{RPL}{\textit{Routing Protocol for Low Power and Lossy Networks}}
\newacronym{hlm}{HLMAC}{Hierarchical Local MAC}
\newacronym{fcc}{FCC}{\textit{Federal Communications Commission}}